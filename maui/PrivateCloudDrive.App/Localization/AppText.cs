...
public static class AppText
{
    ...
    private static readonly IReadOnlyDictionary<string, TextPair> Texts = new Dictionary<string, TextPair>(StringComparer.Ordinal)
    {
        ...
        [nameof(SignInServiceUnavailable)] = new("无法连接服务器，请检查服务器地址或稍后重试。", "Cannot reach the server. Check the server address or try again later."),
        [nameof(SignInNetworkError)] = new("网络连接异常，请检查网络后重试。", "Network connection error. Check the network and try again."),
        [nameof(SignInServerError)] = new("服务器暂时无法完成登录，请稍后重试。", "The server cannot complete sign-in right now. Try again later."),
        [nameof(SignInFailedSafe)] = new("登录失败，请稍后重试或联系管理员。", "Sign-in failed. Try again later or contact an administrator."),
        ...
        [nameof(Upload)] = new("备份", "Back up"),
        [nameof(ThisFolderIsEmpty)] = new("此文件夹为空", "This folder is empty"),
        [nameof(EmptyFolderHelp)] = new("备份照片、文件或创建文件夹来整理当前位置。", "Back up photos and files or create a folder to start organizing this location."),
        ...
        [nameof(SomeUploadsFailed)] = new("部分文件备份失败", "Some backups failed"),
        [nameof(UploadFailed)] = new("备份失败", "Backup failed"),
        [nameof(UploadFailedBeforeRequest)] = new("{0}: 请求到达服务器前备份失败。", "{0}: backup failed before the request reached the server."),
        [nameof(SelectFilesToUpload)] = new("选择要备份的文件", "Select files to back up"),
        ...
        [nameof(ActiveFailedCompletedUploads)] = new("活跃、失败和已完成的备份任务。", "Active, failed, and completed backup tasks."),
        [nameof(NoUploadTasks)] = new("暂无备份任务", "No backup tasks"),
        [nameof(NoUploadTasksHelp)] = new("从文件页或开始备份入口选择照片、视频和文件。等待、备份中、失败和已完成的任务会显示在这里。", "Start backups from Files or the backup action. Waiting, backing up, failed, and completed tasks will appear here."),
        [nameof(GoToFilesToUpload)] = new("去文件页备份", "Go to Files to back up"),
        [nameof(ClearDone)] = new("清除已完成", "Clear Done"),
        [nameof(UploadQueueEmpty)] = new("备份队列为空。", "Backup queue is empty."),
        [nameof(UploadQueueSummary)] = new("{0} 个备份中，{1} 个等待，{2} 个失败，{3} 个已完成。", "{0} backing up, {1} waiting, {2} failed, {3} completed."),
        [nameof(Completed)] = new("已完成", "Completed"),
        [nameof(Waiting)] = new("等待中", "Waiting"),
        [nameof(Uploading)] = new("备份中", "Backing up"),
    ...
    /// 查询指定资源或配置，并返回可被客户端消费的数据模型。
    /// </summary>
    public static string SignInServiceUnavailable => Get(nameof(SignInServiceUnavailable));
    /// <summary>
    /// 查询指定资源或配置，并返回可被客户端消费的数据模型。
    /// </summary>
    public static string SignInNetworkError => Get(nameof(SignInNetworkError));
    /// <summary>
    /// 查询指定资源或配置，并返回可被客户端消费的数据模型。
    /// </summary>
    public static string SignInServerError => Get(nameof(SignInServerError));
    /// <summary>
    /// 查询指定资源或配置，并返回可被客户端消费的数据模型。
    /// </summary>
    public static string SignInFailedSafe => Get(nameof(SignInFailedSafe));
    /// <summary>
    /// 查询指定资源或配置，并返回可被客户端消费的数据模型。
    /// </summary>
