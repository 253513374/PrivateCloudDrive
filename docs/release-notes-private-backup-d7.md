...
| 阻塞 | 状态 | 处理方向 |
| --- | --- | --- |
| main CI / Security Gate 红灯 | 阻断 | DevOps 修复或形成可审计外部豁免 |
| Issue #1/#4/#5 仍 OPEN | 阻断 | 收敛 issue、PR 与发布裁剪说明 |
| Issue #5 安全门禁/secret scan finding | 阻断 | 安全复核、合并/关闭 stale PR、扫描 PASS |
| Android 最终可见验收包未形成单一 PASS 报告 | 阻断 | QA 生成统一证据包并清理过期说明 |
