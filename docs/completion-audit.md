...
- 阶段 8 仍处于 `进行中`。
- 阶段 6.5 已完成 Android 模拟器内测验收，但真实手机和 iOS 体验仍按发布需要补充。
- 阶段 8.2 已接入 Android WeChat SDK 授权桥接，但正式 AppId/AppSecret、Android 签名真机授权、iOS SDK/URL Scheme 和真机授权流程尚未完成。
...
| 阶段 6：MVP Core 产品体验与账号密码认证深化 | `docs/progress.md` 阶段 6 行记录任务 6.1 到 6.4 已落地，账号密码登录失败按用户名和 IP 双维度限流，任务 6.5 手动验收清单和执行记录模板已补充；验证包含后端 build、后端测试 63 个 EF 集成测试、MAUI Windows/Android 构建、临时 API 探针和 password rate limit token endpoint 探针；提交包含 `bce5e4e`。 | 未完成。真实 Android/iOS 设备执行尚未完成。 |
...
| 阶段 8.2：MAUI 微信登录与绑定入口 | MAUI 已有 `IWechatPlatformAuthService`、登录页/设置页入口；Android 已新增 WeChat SDK 授权桥接，Windows/iOS 仍使用默认不可用实现；`docs/progress.md` 记录 Windows/Android 构建通过。 | 部分完成。Android SDK 本地构建完成；真实 Android 授权、正式凭据和 iOS 平台实现仍未验收。 |
...
| 真实 WeChat 端到端验收 | `docs/testing.md` 已新增 V1 微信登录真实设备验收清单，覆盖 Android/iOS 授权、绑定、解绑、锁定、限流和证据记录。 | 未执行。缺正式平台凭据、真实 Android 授权结果、iOS SDK 平台实现和真实设备结果。 |
| 阶段 Git 提交 | `de2c6f9 Checkpoint staged PrivateCloudDrive features` 已提交阶段 5 到阶段 8 的代码、文档和迁移；提交后 `git status --short` 为空。 | 已完成。 |

## 关键未完成项

1. 按发布需要补充真实手机和 iOS 的 MVP Core 验收记录。
2. 准备正式 WeChat `AppId`、`AppSecret`、Android 包名与签名、iOS Bundle Identifier 与 URL Scheme。
3. 在真实 Android 设备上执行 `docs/testing.md` 的 V1 微信登录真实设备验收清单。
4. 接入 iOS WeChat SDK，并在真实 iOS 设备上执行授权、绑定和解绑验收。
5. 真实设备执行完成后，按阶段完成规则继续更新验收记录并提交。

## 当前不能作为完成证明的信号

- 后端测试通过不能证明真实设备 WeChat 授权链路完成。
- Android SDK 构建通过不能证明真实微信客户端授权、平台签名和后端 code 交换已通过。
- `docs/testing.md` 中已有验收清单不能替代真实设备执行结果。
- `de2c6f9` 或 Android SDK 接入提交不能替代真实 Android/iOS 设备验收和真实 WeChat 授权链路。
