...
- 阶段 5 到阶段 8 均仍处于 `进行中` 或存在未提交状态。
- 阶段 6.5 真实设备 MVP Core 验收尚未执行。
- 阶段 8.2 真实 WeChat Android/iOS SDK、正式 AppId/AppSecret、Android 签名、iOS URL Scheme 和真机授权流程尚未完成。
- 当前工作区存在大量未提交和未跟踪文件，阶段完成规则要求提交 Git，但尚未进行提交整理。

## 要求到证据清单

| 要求 | 证据 | 当前状态 |
| --- | --- | --- |
| 阶段 5：MVP Core 回收站、部署与质量收尾 | `docs/progress.md` 阶段 5 行记录回收站 API 与 App 入口、Docker Compose、README、部署说明和测试说明已完成收尾复核；验证包含后端 build、后端测试、MAUI Windows/Android 构建和 `docker compose config`。 | 未完成。阶段行仍为 `进行中`，对应提交为 `未提交`。 |
| 阶段 6：MVP Core 产品体验与账号密码认证深化 | `docs/progress.md` 阶段 6 行记录任务 6.1 到 6.4 已落地，任务 6.5 手动验收清单已补充；`docs/testing.md` 包含移动端真实设备手动验收清单。 | 未完成。真实 Android/iOS 设备执行尚未完成，对应提交为 `未提交`。 |
| 阶段 7：V1 分享、标签、收藏与操作日志 | `docs/progress.md` 阶段 7 行记录分享、标签、收藏、媒体库和操作日志后端与 MAUI 入口已接入；验证包含后端测试、临时 API 探针和 MAUI Windows/Android 构建。 | 未完成。阶段仍为 `进行中`，仍有 `未提交` 工作。 |
| 阶段 8.1：微信登录后端基础 | `docs/progress.md` 阶段 8 行记录 WeChat 配置、`WechatUserBinding`、绑定/解绑接口、绑定票据、OpenIddict 自定义 grant、审计、唯一索引、lockout 和限流已实现；验证包含 60 个 EF 测试、后端 build、DbMigrator 和临时 API 探针。 | 后端主体已完成本地验证，但阶段未完成。 |
...
| 阶段 Git 提交 | `git status --short` 显示大量已修改和未跟踪文件，包含阶段 5 到阶段 8 的代码、文档和迁移。 | 未完成。提交整理尚未执行。 |
...
- `docs/progress.md` 中阶段 5 到阶段 8 的实现记录不能替代阶段 Git 提交。

## 最新预提交验证证据

以下验证已在 2026-05-08 刷新，说明当前工作区具备提交前的基础质量条件，但不代表目标整体完成：
...
在没有正式 WeChat 平台凭据和真机设备前，最可执行的下一步是提交整理：先按阶段拆分当前工作区变更，排除生成物和运行时目录，再按阶段完成规则提交。
