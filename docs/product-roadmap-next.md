...
**状态：✅ 已发布（2026-07-14，见 `docs/release-notes-v1.4.md`）**

**发布范围：**
- ✅ UX-01（搜索入口）、UX-03（容量可视化）、UX-04（排序筛选）— 已合并（PR #78/#79/#80）
- ✅ UX-02（批量操作 MAUI）— 已合并（PR #86）
- ✅ KN-01~03（缓存提示 + 角色选择器）— 已合并（PR #85）
- ✅ G1（MAUI 编译）— 已修复并合并（PR #88），`dotnet build -f net10.0-android` 0 errors
- ✅ QA-01（真机验收）— API 级 17/19 PASS，2 项 WARN（MAUI 平台限制）
- 🔲 P1 项（UX-05~08）— 已降级至 V1.5

| 优先级 | 功能/任务 | 说明 | 验收标准 | 当前状态 |
|--------|-----------|------|---------|:--------:|
| P0 | 搜索体验闭环 | 搜索 API 已有，补齐前端的搜索入口、搜索结果页、空状态、清除搜索 | 各文件页顶部搜索框可用，搜索结果分页正确且无跨用户泄露 | ✅ **已合并** |
| P0 | 批量操作 MAUI 体验 | 多选模式、全选、批量删除/恢复/永久删除，危险操作二次确认 | 10+ 文件可批量选择并操作，有进度反馈和二次确认弹窗 | ✅ **已合并** |
| P0 | 容量可视化 | Settings 已占空间/配额/可用空间，上传超限提示 | 用户能清晰看到已用容量和配额，超限时不可上传且有提示 | ✅ **已合并** |
| P0 | 排序与筛选 UI | 名称、时间、大小、类型；按文件类型/收藏/标签筛选 | 文件页可通过底部弹窗切换排序和筛选条件 | ✅ **已合并** |
| P0 | 已知限制 P0 修复 | KN-01~03：缓存失效提示、健康缓存说明、创建用户角色选择器 | 每条修复有独立 AC 和回归验证 | ✅ **已合并** |
| P0 | 真机验收补完 | Android 真机主链路（登录→浏览→上传→下载→预览→删除→恢复→分享） | 真实设备完成全链路记录，G4 从 FAIL 升级为 PASS | ⚠️ **有条件 PASS**（17/19） |
| P0 | MAUI 编译修复 | 上传取消代码 6 个编译错误修复（CS0111/CS1503/CS1061） | `dotnet build -f net10.0-android` 0 errors | ✅ **已修复**（PR #88） |
| P1 | 分享管理增强 | 我的分享列表、复制链接、取消分享、密码状态、访问统计 | Settings 中分享管理入口可用 | 🔲 **未开始**（V1.4 已降级） |
| P1 | 上传队列重试/取消 | 上传失败可重试，上传中可取消，错误信息可理解 | 上传页可操作单个上传任务 | 🔲 **未开始**（V1.4 已降级） |
| P1 | 媒体库交互完善 | 图片时间线交互增强、视频列表信息完整 | 媒体库页面交互流畅 | 🔲 **未开始**（V1.4 已降级） |
| P1 | 设置页 IA 最终调优 | 管理员/普通用户入口差异化、操作路径一致性检查 | 多角色测试无链路断裂 | 🔲 **未开始**（V1.4 已降级） |

**明确不做：**
- 多用户/家庭空间 → 已移入 V2.0 范围（见 4.6）

### 4.6 V2.0：家庭空间 / 团队空间 / 智能能力

**状态：🚀 已授权（Phase-0 设计冻结进行中，见 `docs/release-plan-v2.0.md`）**

V2.0 方向已于 2026-07-16 经用户授权确认。

**MVP 范围：家庭/团队空间底座**（从个人 OwnerId 云盘升级为 Space 云盘的架构版本）

**Phase-0 进行中：**
- ADR-01: Space 领域实体设计（architect）
- ADR-02: 空间权限模型设计（architect）
- ADR-03: 文件归属升级策略 OwnerId → SpaceId（architect）
- ADR-04: 数据库迁移与回滚策略（db-dba）
- PRD 完善：V2.0 MVP 产品规格细化（pm）
