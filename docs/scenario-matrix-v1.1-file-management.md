...
基线来源：`docs/scenario-matrix-v1.0-rc.md`（V1.0 RC 场景矩阵）
...
V1.1 阶段的目标是把 V1.0 RC 的"文件能上传和访问"升级为"能快速找到、批量整理、理解容量占用、管理分享"的私有云盘体验。本场景矩阵覆盖以下 7 个用户旅程：
...
| 规则编号 | 规则 | 原因 |
|----------|------|------|
| BR-CAP-01 | 容量摘要返回用户级已用字节 + 全盘配额（如果已配置）+ 剩余字节 | V1.1 以全盘为主，用户级配额为 V1.3 目标 |
...
| 下游角色 | 交付物 | 说明 |
|----------|--------|------|
| 后端 (backend-eng) | US-V11-01 至 US-V11-07 的接口扩展/新增（§6 接口矩阵）+ 业务规则 | 搜索 ILIKE 实现、批量操作 DTO、容量 API、分享禁用、重命名移动 |
| 移动端 (mobile-eng) | 各场景的「用户可见信息」列 + 页面状态定义（§5）+ 正常路径流程 | 搜索框、排序弹窗、多选 UI、容量卡片、分享列表、上传队列、重命名/移动弹窗 |
| 测试 (qa-eng) | 各场景的「验收条件」表 + 异常路径表 | 可直接映射为测试用例 |
| UX | 「页面状态定义」§5 的 11 种状态 | 空/错/加载/多选/容量不足等状态的交互规范 |
| 安全 (security-reviewer) | 权限矩阵（§7）、数据字典安全约束（§8） | 搜索隔离、批量权限、容量数据可见范围 |
