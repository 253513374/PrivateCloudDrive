...
V1.4 是文件管理体验增强版（UX Polish & Known Limitations Fix），不引入后端架构变更。以下限制在 V1.4 范围内已知，发布时需同步确认（来源：`docs/release-notes-v1.4.md`）。

| 编号 | 限制 | 影响 | 状态 | 规避/备注 |
|:----:|------|------|:----:|-----------|
| KN-V1.4-01 | 搜索使用 PostgreSQL ILIKE（NormalizedName.Contains）而非全文搜索引擎；大目录（10 万+文件）未实测 | 搜索性能 | ✅ 已确认 | 个人/家庭规模性能充足；全文索引列为 V1.5 候选 |
| KN-V1.4-02 | 批量操作单次最多 100 个文件，不支持跨目录多选 | 批量范围 | ✅ 已确认 | UX-02 实现约束，满足日常批量删除/恢复/永久删除 |
| KN-V1.4-03 | 容量可视化数据来自 API 调用，非实时推送更新 | 容量展示时效性 | ✅ 已确认 | 页面加载时刷新获取最新值；上传超限时实时提示 |
| KN-V1.4-04 | MAUI Android 编译存在 G1 阻塞（上传取消功能代码不完整导致 6 个编译错误） | 构建可用性 | ⚠️ **阻塞** | 编译错误（CS0111/CS1503/CS1061 等），需 mobile-eng 修复后 V1.4 才能通过 G1 门禁 |
| KN-V1.4-05 | Android 真机验收已完成 API 级验证（17/19 PASS），设置面板入口（Q）和故障诊断展开区（S）因 MAUI 不暴露 Android Accessibility 树无法在自动化框架中验证 | 验收覆盖 | ⚠️ 有条件 PASS | G4 门禁有条件升级；保留 TD-01（MAUI UI 自动化框架）为技术债务 |
| KN-V1.4-06 | 排序与筛选状态在页面切换后可能丢失（UX-04 前端实现约束） | 用户体验 | ⚠️ 待确认 | 需在 V1.4 真机验收中验证，V1.5 改进 |
| KN-V1.4-07 | iOS 客户端不在 V1.4 范围内；MAUI 构建仅验证 Android 目标 | 平台覆盖 | ✅ 延续 V1.3 | 如需 iOS，请等待后续版本 |
