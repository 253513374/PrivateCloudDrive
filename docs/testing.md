...
|------|-----------|:------:|---------|:------------:|:----:|------|
| AC-V1.3b-01 | 范围冻结 | P0 | 本轮差异不包含新增后端 API、数据库表、认证流、存储抽象、上传下载主链路、媒体 pipeline 或分享公开访问语义变更 | KN-V1.3b-01 | ✅ PASS | 通过 `git diff --name-only` 和代码评审确认 |
| AC-V1.3b-02 | `known-limitations.md` 同步 | P0 | 已包含 release-notes-v1.3.md 的 KN-V1.3-01~11，并追加 V1.3b 客观限制 | KN-V1.3b-03 | ✅ PASS | 不删除历史限制；4 条 V1.3b KN 全部确认 |
| AC-V1.3b-03 | Settings 管理区角色可见性 | P0 | 管理员登录可见 8 项管理入口；普通用户不可见管理区；健康状态圆点对所有用户可见 | KN-V1.3-07 | ⚠️ 条件 PASS | F-05: 代码已在 PR #64 合并且确认通过，但 Android 模拟器自动化截图 2×60 迭代均超时 (qa-eng)，待人工补采 settings-admin.png / settings-regular.png |
...
**验收条件**：API 级验证 + UI 截图验证。当前仅完成 API 级验证，UI 截图待人工补采。

| # | 验收项 | 期望结果 | 结果 | 备注 |
|:-:|--------|---------|:----:|------|
| A | 登录 | 正确凭据后登录成功 | ✅ **PASS** | API 级验证 |
| B | token 续期 | 关闭再打开不需重新登录 | ✅ **PASS** | API 级验证 |
| C | 文件列表浏览 | 目录正确，滚动流畅 | ✅ **PASS** | API 级验证 |
| D | 小文件上传 | 上传成功 | ✅ **PASS** | API 级验证 |
| E | 大文件上传 | 分片上传完成 | ✅ **PASS** | API 级验证 |
| F | 文件下载 | 可打开 | ✅ **PASS** | API 级验证 |
| G | 图片预览 | 缩略图 + 全屏预览 | ✅ **PASS** | API 级验证 |
| H | 视频播放 | 封面 + 播放 | ✅ **PASS** | API 级验证 |
| I | 删除到回收站 | 出现在回收站 | ✅ **PASS** | API 级验证 |
| J | 从回收站恢复 | 回到原目录 | ✅ **PASS** | API 级验证 |
| K | 永久删除 | 从回收站彻底删除 | ✅ **PASS** | API 级验证 |
| L | 创建分享 | 链接可访问 | ✅ **PASS** | API 级验证 |
| M | 密码分享 | 密码才可访问 | ✅ **PASS** | API 级验证 |
| N | 图片时间线 | 按月份/日期分组 | ✅ **PASS** | API 级验证 |
| O | 视频列表 | 时长/分辨率/封面 | ✅ **PASS** | API 级验证 |
| P | 搜索功能 | 搜索结果正确 | ✅ **PASS** | API 级验证 |
| Q | 设置页各面板入口 | 管理员 8 项可访问 | ⚠️ **WARN** | MAUI Accessibility 限制，非该版本可修复 |
| R | 系统健康页 | 各组件状态正确 | ✅ **PASS** | API 级验证 |
| S | 故障诊断展开区 | 6 个展开区可操作 | ⚠️ **WARN** | MAUI Accessibility 限制，非该版本可修复 |

**QA-01 结果**：**17 PASS / 2 WARN（Q/S）** — API 级验证通过，UI 交互项因 MAUI 平台限制标记 WARN。
...
| 验收项 | 操作 | 预期结果 | 结果 |
|--------|------|---------|:----:|
| AC-UX01-D | 用户 A 搜索，验证结果不包含用户 B 文件 | ILIKE + CurrentUser 过滤，数据隔离 | ✅ **PASS**（API 层面已审计） |
