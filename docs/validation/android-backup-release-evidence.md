...
结论：当前 `docs/validation/` 已具备一组可公开引用的 Android 模拟器端到端可见证据，覆盖备份入口、照片/文件备份、批量/队列状态、失败重试、下载/预览、容量/健康、恢复说明和隐私边界。证据包可作为 D7 发布闸门输入；真机相册权限、后台长任务、弱网与 OEM 差异仍列为发布后增强或真机补证项。
...
| 下载/预览 | PASS，Android 页面可见下载列表；后端恢复烟测覆盖 Range 下载和内容预览读取 | `docs/validation/screenshots/private-backup-slice4-downloads-list.png`；`docs/validation/backup-restore-destructive-test-stack-20260521-215020.md` | 可写为“下载列表可见，恢复烟测已覆盖下载/预览控制路径”。 |
| 删除/恢复 | PASS，后端灾备烟测覆盖删除到回收站、回收站列表和恢复；Android 设置页存在回收站入口 | `docs/validation/backup-restore-destructive-test-stack-20260521-215020.md`；`docs/validation/screenshots/private-backup-slice3-settings-health.png` | 可写为“恢复能力已由后端灾备烟测证明，Android 入口可见；真机端删除/恢复全链路建议继续补证”。 |
...
| 入口 | 状态 | 说明 |
|---|---|---|
| `docs/validation/daily-acceptance-20260523-review-7/validation-evidence-index.md` | PASS，Sensitive findings 0 | 包含 10 个证据文件，其中含公开敏感扫描摘要。 |
...
说明：部分底层报告中保留历史 WARN/FAIL 计数字样用于说明旧限制或早期演练缺口，不等同于 D7 证据包当前敏感信息闸门失败。发布引用时以 daily acceptance index 的 `Status: PASS` 和 `Sensitive findings: 0` 为闸门摘要，同时在“已知限制”章节列出未覆盖项。
...
```bash
git diff --check
grep -RInE 'access_token|refresh_token|cookie|password|client_secret|Bearer [A-Za-z0-9._-]+' docs/validation --include='*.md' --include='*.txt' --include='*.log' || true
```

如 grep 命中，应人工确认是否为规则说明文字；命中真实敏感值时 release gate 必须改为 FAIL，并先移除/轮换。
